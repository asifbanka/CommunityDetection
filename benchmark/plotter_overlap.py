...
def plotFile(filename, round):
    ...
    with open(filename) as infile:
        ...
        #label += ", round=" + str(i)
        title = str(data["_mu"]) + "mu "
        title += str(data["_N"]) + "N"
        nmiValuesMean = data["nmiValuesMean"]
        for tup in nmiValuesMean:
            xs.append(float(tup["on"])/float(data["_N"]))   #the devision by N because the files hat (erroneosly) stored the actual number of overlapping nodes instead of the fraction
            ys.append(float(tup["value"][i]))
    return xs, ys, label, title
...
def commandline_interface():
    ...
    parser.add_option("-f", dest="files", type="string",
        help="Whitespace-separated list of json-files. This program plots NMI (y-axis) against the overlap (x-axis). \
...
if commandline_interface():
    ...
    for f in filenames:
        if(options.iterations == -1):
            xs, ys, label,title = plotFile(f, -1)
            line, = plt.plot(xs, ys, "-o", label=label)
            lines.append(line)

        else:
            xs, ys, label,title = plotFile(f, 0)
            line, = plt.plot(xs, ys, "-o", label=label)
            lines.append(line)

            xs, ys, label,title = plotFile(f, options.iterations)
            line, = plt.plot(xs, ys, "-o", label=label)
            lines.append(line)

    plt.legend(fancybox=True, shadow= True, loc=3)
    plt.axis([0,0.6,0,1])
    plt.xlabel("overlap")
