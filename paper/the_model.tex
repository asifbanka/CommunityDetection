...
The result of the (infinite) walk itself can be expressed as $\lim_{k \rightarrow \infty}{\mat{P}^k}$.
...
We also know that there are~$k$ (possibly overlapping) communities which we want to discover.
...
Now $\mat{P}(i, j)$ holds the transition-probability from node $v_i$ to $v_j$.
...
Furthermore, can assume that the nodes of~$G'$ to be ordered as 
...
It follows directly from the construction of the graph $G'$ and the fact that the original graph was connected, 
that random walks in $G'$ define an absorbing Markov chain. Here, the absorbing states correspond to the set of seed nodes.
...
Since we are dealing with infinite random walks we summarize the following 
important property of absorbing Markov chains, which can be shown by a geometric argument.
\begin{proposition}\label{prop:limiting_Q}
    ...
    \end{equation}  
\end{proposition}
This states that random walks in our graph $G'$ actually converge to a meaningful value.
For random walks in other graphs this may not necessarily be the case.
Also, since $\lim_{r \to \infty} \mat{Q}^r = \mat{0}$, the probability that a random walk starting anywhere in the graph
eventually gets trapped at one of the sink nodes, i.e., seed nodes is 1.
Furthermore, the probability that such an infinite random walk starting at non-seed node~$u_i$ ends up at the seed 
node~$x_j$ is contained in the the entry $(i, j)$ of the submatrix $\mat{X} := (\mat{I} - \mat{Q})^{-1} \cdot \mat{R}$. 
...
The computational complexity of calculating these affinity values now 
depends on how efficient we can calculate the values of $\mat{X}$, i.e., solve $(\mat{I} - \mat{Q})^{-1}$.
In the next subsection, we show how to reduce this problem to that of solving a 
