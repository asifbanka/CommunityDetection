...
The non-seed node then adopts the affinities of the seed nodes such a random walk would be likely to reach first.

Each step of a random walk can be represented by a transition matrix $P$ 
and the result of the (infinite) walk itself can be expressed as $\lim_{k \rightarrow \infty}{P^k}$.
...
\begin{equation}\label{eqn:final_affinity}Voraussichtlicher Liefertermin: Samstag, 22. November 2014 - Donnerstag, 27. November 2014
...
Note that computing $\sum_{j = 1}^{s} \alpha (x_j, l) \cdot \mat{R}_j$ takes time $O(m)$, where $m$ 
denotes the number of non-zero entries in  $\mat{P}$\footnote{This is almost the same as the 
number of edges in $G$, but not quite, since while constructing $\mat{P}$ from the graph $G$, 
we add self-loops on seed nodes and delete edges between adjacent seed nodes, if any. However the number 
of non-zero entries of \mat{P} is at most $|E(G)| + s \leq |E(G)| + |V(G)|$.}. 
Computing the product of $\mat{D}_1$ and $\sum_{j = 1}^{s} \alpha (x_j, l) \cdot \mat{R}_j$ 
takes time $O(m)$ so that the right hand side of equation~(\ref{eqn:final_affinity}) can 
be computed in time $O(m)$. We now have a symmetric diagonally dominant system of linear equations 
which by Proposition~\ref{prop:SDD_systems} can be solved in time $O(m \cdot \log n)$. Therefore 
the time taken to compute the affinity to a fixed community is $O(m \cdot \log n)$,
