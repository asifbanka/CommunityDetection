...
$\mat{D}_1$ and $\sum_{j = 1}^{s} \alpha (x_j, l) \cdot \mat{R}_j$ takes time $O(m)$ so that 
the right hand side of equation~(\ref{eqn:final_affinity}) can be computed in time $O(m)$.
