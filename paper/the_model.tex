We assume that the complex networks that we deal with are modeled as undirected graphs.  
In broad brush-strokes, this is how our algorithm works.  The algorithm receives 
as input a network and a set of nodes from each community that we are aiming to 
discover. We call these nodes \emph{seed nodes}. A seed node can belong to several 
communities and our algorithm actually finds out overlapping communities in the 
network. The algorithm uses the partial community information and propagates it 
to the other nodes of the network by simulating a random walk from non-seed nodes 
to the set of seed nodes.  

Suppose that $x_1, \ldots, x_s$ are the seed nodes in the network and let $u$ 
be a non-seed node. We consider random walks starting at $u$ and of lengths 1, 2, till 
infinity and estimate the probability $p_1, \ldots, p_s$ of any such walk ending up at the 
seed nodes $x_1, \ldots, x_s$, respectively. If out of these~$s$ seed nodes, 
$x_{i_1}, \ldots, x_{i_r}$ belong to a community~$C$, then the algorithm assigns 
a probability of $\sum_{j = 1}^r p_{i_j}$ to the event that~$u$ belongs to~$C$. 
Therefore for each community, we assign a probability for the event that~$u$ belongs 
to it. \texttt{The communities that the algorithm assigns~$u$ are those for which 
the probabilities exceed a certain threshold. True??}  
...
systems of linear equations (with one set of equations per community), 
...
In practical situations, we have~$k \ll n$. Moreover, we have some partial information 
about each community in that we know $t$ members of each community. As already mentioned, 
we call these the \emph{seed nodes} while the remaining nodes are
\emph{non-seed nodes}. Together, we know the community information 
of $s :=  k \cdot t$ seed nodes. The community information of a node~$u$ is represented 
...
\begin{equation}\label{eqn:defining_prob_nonseed}
	\mat{P}(u, z) = \left \{ 
							\begin{array}{ll}
								\frac{1}{\deg_G (u)} & \mbox{ if } \{u, z\} \in E(G) \\
								0			& \mbox{ otherwise,}
							\end{array}
						\right.
\end{equation}
where $\deg_G (u)$ is the degree of node $u$ in the network~$G$.
