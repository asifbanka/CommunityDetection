...
worthwhile to study how well our algorithm performs 
at least in special cases.

Suppose that the network in question has a community, which we call $C_1$,
with $n_1$ nodes and $e_1$ internal edges (edges with both end-points within 
the community). Community $C_1$ is separated from the ``rest of the network,'' 
which we call $C_2$. We assume that $C_2$ has $n_2$ vertices and $e_2$ internal 
edges and that $e_3$ edges connect $C_1$ and $C_2$; that is, the cut $(C_1, C_2)$
has $e_3$ edges. 
