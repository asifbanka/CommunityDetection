...
systems of linear equations (one set per community), which by the work of Speilman and 
Teng~\cite{ST04,EEST05,ST08,Vis13} can be done in $O(m \log n)$ time, where~$m$ is the number of edges
in the graph. We assume that our networks are sparse in the sense that $m = O(n)$, 
so that the running time of our algorithm can  be bounded by $O(n \log n)$.    
