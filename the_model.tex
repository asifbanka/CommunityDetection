...
or a collection of communities. We wish to ascertain the probability 
with which a random walk starting at a randomly chosen node in $C_1$ ends up in 
a seed node for $C_1$. Assume that we pick a set $S_1$ of $s_1$ seed nodes from 
$C_1$ and a set $S_2$ of $s_2$ seed nodes from the rest of the graph, uniformly 
at random. Now the probability that a randomly picked node~$u$ from $C_1$ 
transitions to one of the seed nodes in $S_1$ in \emph{one} step is:
\begin{align}
...
we choose to ignore this here because, for one, the probability expressions 
are much simpler this way and the real objective here is to gain intuition as to 
...
The probability that a random non-seed node of $C_1$ is absorbed by a 
...
\end{equation}
The probability that a random non-seed node of $C_1$ is absorbed by 
...
\begin{equation} \label{exp:final_ratio}
    \frac{s_1}{n_1} \left ( \frac{n_2}{s_2} \cdot 
            \frac{2e_1 + e_3}{2e_2 + e_3} + 
            \frac{2e_1 + e_3}{e_3}\right ).
\end{equation}
