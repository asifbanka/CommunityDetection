...
We wish to determine the matrix $\mat{N} \cdot \mat{R}$. Entry $(u, x)$ of this 
matrix is the probability that a random walk starting at~$u$ ends up at~$x$. The 
belonging vector of $u$ would then be computed as:
\begin{equation}\label{eqn:belonging_vector}
	\beta(u) = \sum_{i = 1}^{s} p(u, x_i) \beta(x_i).
\end{equation}
