...
the community). Community $C_1$ is separated from the ``rest of the network,'' ($C_2$)
by $e_3$ edges; that is, the cut $(C_1, C_2)$ has $e_3$ edges. We assume that $C_2$ 
has $n_2$ vertices and $e_2$ internal edges. Now $C_2$ could be a community in itself 
or a collection of communities. We are interested in ascertaining the probability 
with which random walks starting at a randomly chosen node in $C_1$ end up in 
a seed node for $C_1$. Assume that we pick a set $S_1$ of $s_1$ seed nodes from 
$C_1$ and a set $S_2$ of $s_2$ seed nodes from the rest of the graph, uniformly at random. Now the 
probability that a randomly picked node~$u$ from $C_1$ transitions to one of 
the seed nodes in $S_1$ in one step is:
\begin{align}
    \PR{u \in_{\mathrm{u.a.r}} C_1}{u \to S_1} 
                    & = \frac{1}{d(u)} \sum_{x \in N(u)} \frac{s_1}{n_1} \nonumber \\
                    & = \frac{s_1}{n_1}.
\end{align}
This is not entirely correct, since vertex $u$ may have neighbors in $C_2$. However 
we choose to ignore this inaccuracy because, for one, the probability expressions 
are much simpler and the real objective here is to gain intuition as to 
why the algorithm might succeed in uncovering communities. 

The probability with which a randomly chosen node in $C_1$ transitions to a 
node in $C_1$ (in one step) is 
\[
...
\begin{equation}\label{exp:c1-c2}
    \frac{e_3}{2e_1 + e_3}.
\end{equation}
The probability expression~(\ref{exp:c1-c1}) 
can be broken down into two events: the probability 
of transitioning to a seed node in $S_1$ (which is $s_1 / n_1$), 
and the probability of transitioning to a non-seed node in $C_1$, 
which then is:
...
where we used bars on $C_1$ and $C_2$ to denote the non-seed node counterparts. 
...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2.8cm,
  ...
  \path (S1) edge [loop above]  node {$1$}                                        (S1)
        ...
        (C1) edge [bend left]   node (dummy1) [above] {$\frac{e_3}{2e_1 + e_3}$}  (C2)
             edge [loop left]   node {$\frac{e_3}{2e_1 + e_3} - \frac{s_1}{n_1}$} (C1)
        (C2) edge [bend left]   node (dummy2) [below] {$\frac{e_3}{2e_2 + e_3}$}  (C1)
             edge [loop right]  node {$\frac{e_3}{2e_2 + e_3} - \frac{s_2}{n_2}$} (C2)
        (C2) edge               node {$\frac{s_2}{n_2}$}                          (S2)
        (S2) edge [loop above]  node {$1$}                                        (S2);
        
\end{tikzpicture}
\end{center}
\caption{The four-state Markov chain representing community information}
\label{fig:4stateMC}
\end{figure}
