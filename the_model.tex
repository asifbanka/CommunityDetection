...
\end{equation}
The leading term in the above expression is $s_1/ n_1$ and if we assume that 
$\partial C_1 / n_1$ is small enough, then we can neglect all the other 
terms of this expression. We therefore assign a probability of $s_1 / n_1$
to this event.

The probability expression~(\ref{exp:c1-c1}) can be broken down 
...
Up till now, we have made no assumption abut community structure. In particular,
we did not make the commonly assumed characteristic that communities 
and more dense inside than outside. We make this 
