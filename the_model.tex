...
The \emph{Laplacian} of an undirected graph~$G$ is defined to be $\mat{L} = \mat{D} - \mat{A}$, 
where $\mat{A}$ is the adjacency matrix of the graph~$G$ and $\mat{D}$ is the 
diagonal matrix of vertex degrees. We assume that $G$ is unweighted but one can 
generalize the definition to the weighted case.  
...
Given an $n \times n$ matrix~$\mat{D}$ and $\vect{e} \in \R^{n}$, consider the 
system of linear equations $\mat{D} \vect{x} = \vect{e}$, with variables
$\vect{x} = \trans{(x_1, \ldots, x_n)}$. Such a system has a solution if and only if
$\vect{e}$ is in the image of matrix~$\mat{D}$. If $\mat{D}$ is invertible, then 
the system has a solution for all $\vect{e} \in \R^n$. However, if $\vect{e}$ 
is in the image of $\mat{D}$, then the inverse of $\mat{D}$ is well-defined 
and is denoted by $\pseudo{\mat{D}}$. This is called the \emph{pseudo-inverse}
of $\mat{D}$. 
\begin{proposition} \label{prop:Laplacian_systems} {\textrm{\cite{Vis13}}}
...
We can use Proposition~\ref{prop:Laplacian_systems} to upper-bound the time taken to 
...
\end{lemma}  
\begin{proof}
Recall that the we ordered the nodes of $G$ as $\{u_1, \ldots, u_{n - s}, x_1, \ldots, x_s\}$, 
...
of~$G$. Let $\mat{A}_1$ and $\mat{D}_1$ denote, respectively, the adjacency matrix and the 
diagonal matrix of vertex degrees of $G_1$. Then we can express 
$\mat{I} - \mat{Q}$ as 
\[
	\mat{I}  - \mat{Q} = \inv{\mat{D}_1} (\mat{D}_1 - \alpha_1 \mat{A}_1).
\]   
\end{proof} 
