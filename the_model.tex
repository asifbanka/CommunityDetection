...
The \emph{Laplacian} of an unweighted, undirected graph~$G$ is defined to 
be 
\[
\mat{L}_G = \mat{D}_G - \mat{A}_G,
\] 
where $\mat{A}_G$ is the adjacency matrix 
of the graph~$G$ and $\mat{D}_G$ is the diagonal matrix of vertex degrees. 
A weighted graph $G = (V, E)$ has an associated weight function 
...
the degree~$\deg_G (u)$ of a vertex~$u$ is defined to be 
...
\]   
Note that $\mat{D}_1 - \mat{A}_1$ is not really the Laplacian of the graph $G_1$
but is symmetric and weakly diagonally dominant. Let us suppose that $\mat{X}$ 
is an $(n - s) \times s$ matrix such that 
\[
...
By Proposition~\ref{prop:Laplacian_systems}, the time taken to solve each of these systems 
